...
async function queryEntityRelations(
  ...
  const matchedOut = outgoing.filter(r =>
    relationTypes.some(rt => r.relation.toUpperCase().includes(rt.toUpperCase()))
  );
  const matchedIn = incoming.filter(r =>
    relationTypes.some(rt => r.relation.toUpperCase().includes(rt.toUpperCase()))
  );
...
async function queryEntityProperties(
  ...
  const outgoing = engine.getRelationsFrom(entity.id);
  const roleRelations = outgoing.filter(r =>
    ['LEADS', 'MANAGES', 'HEADS', 'WORKS_AS', 'ROLE_IS'].includes(r.relation.toUpperCase())
  );
...
async function queryWhoAtEntity(
  engine: GraphEngine,
  entityName: string,
  config: Config
): Promise<NLQueryResult> {
  ...
  const allConnected = [...incoming, ...outgoing]
    .filter(r => r.from_type?.toLowerCase() === 'person' || r.to_type?.toLowerCase() === 'person')
    .map(r => r.from_type?.toLowerCase() === 'person'
      ? { name: r.from_name, type: r.from_type }
      : { name: r.to_name, type: r.to_type }
    );
...
async function queryWhoDidEntity(
  engine: GraphEngine,
  verb: string,
  entityName: string,
  config: Config
...
async function querySmartFallback(
  engine: GraphEngine,
  question: string,
  config: Config
): Promise<NLQueryResult> {
...
async function queryByRelationPattern(
  engine: GraphEngine,
  relationHint: string,
  config: Config
): Promise<NLQueryResult> {
...
async function queryWhoRelation(
  engine: GraphEngine,
  relation: string,
  entityName: string,
  config: Config
...
async function queryAboutEntity(
  engine: GraphEngine,
  entityName: string,
  config: Config
): Promise<NLQueryResult> {
