...
async function queryWhoRelation(engine, relation, entityName, config) {
  ...
  const incoming = engine.getRelationsTo(entity.id);
  const people = incoming.filter((r) => r.from_type.toLowerCase() === "person").map((r) => ({ name: r.from_name, type: r.from_type }));
  const outgoing = engine.getRelationsFrom(entity.id);
  const outPeople = outgoing.filter((r) => r.to_type.toLowerCase() === "person").map((r) => ({ name: r.to_name, type: r.to_type }));
  const all = [...people, ...outPeople];
  const unique = [...new Map(all.map((r) => [r.name, r])).values()];
  return {
    answer: unique.length > 0 ? `${unique.map((p) => p.name).join(", ")} \u2192 ${entityName}` : `No people found connected to "${entityName}".`,
    entities: unique,
