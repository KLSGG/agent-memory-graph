...
import type { ExtractedEntity, ExtractedRelation, ExtractionResult } from './extractor.js';
...
const TYPE_INDICATORS: Record<string, RegExp[]> = {
  Person: [
...
export function localExtract(text: string): ExtractionResult {
  ...
  for (const rp of RELATION_PATTERNS) {
    ...
    while ((relMatch = rp.pattern.exec(text)) !== null) {
      ...
      if (STOP_ENTITIES.has(from) || STOP_ENTITIES.has(to)) continue;
 ...
 */
export function needsLLMExtraction(text: string, localResult: ExtractionResult): boolean {
