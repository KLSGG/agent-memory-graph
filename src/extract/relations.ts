 ...
 */
export function normalizeRelation(relation: string): string | null {
 ...
 */
export function isVagueRelation(relation: string): boolean {
  const normalized = relation.trim().toUpperCase().replace(/[\s-]+/g, '_');
 ...
 */
export function getCanonicalRelation(relation: string): string {
  const normalized = relation.trim().toUpperCase().replace(/[\s-]+/g, '_');
  return RELATION_SYNONYMS[normalized] || normalized;
}
