...
export interface DedupResult {
  normalized: number;
  removed: number;
  mergedRelations: { from: string; to: string; count: number }[];
}
 ...
 */
export function dedupRelations(db: Database.Database): DedupResult {
  const result: DedupResult = { normalized: 0, removed: 0, mergedRelations: [] };

  // Get all relationships
  const allRels = db.prepare(`SELECT * FROM relationships`).all() as any[];

  const now = new Date().toISOString();
  const toDelete: string[] = [];
  const toUpdate: { id: string; relation: string }[] = [];
